...
(function($) {
    ...
    ElementContent = (function() {
        ...
        function decodeShingles(bytearr) {
            ...
            for(var i = 0; i < bytearr.length; i += 2) {
                ...
                shingles.push({ value: first_char + '' + second_char, freq: freq + 1 });
            }    
        ...
        ElementContent.fromEncodedFreqs = function(bytearr) {
            ...
            if((bytearr[0] & 0x80) === 0x80) {
                result.shingles = decodeShingles(bytearr.slice(1, content_len + 1));
            }        
        ...
        ElementContent.prototype = {
            getShingleFreqs: function() {
                if(typeof this.shingles === 'undefined')
                    this.shingles = getShingleFreqs(this.element);
                
                return this.shingles;    
            },
            getDescendantFreqs: function() {
                if(typeof this.descs === 'undefined')
                    this.descs = getDescendantFreqs(this.element);
    ...
    $.xJQ = function(selector) {
            ...
            for(var b = 0x80; b > 0x00 && !stopped; b = b >>> 1) {
                ...
                if(bits_to_buffer > 0) {
                    ...
                    if(bits_to_buffer === 0 && last_keyword === KEYWORD_NUM) {
                        result.last().setPosition(buffer); 
                        bits_to_buffer = 0;
                        st_pos = st_keywords; 
                    }
                } else {
                        ...
                        switch(mode) {
                            case MODE_KEYWORD: {
                                ...
                                switch(+found) {
                                    ...
                                    case KEYWORD_ID: {
                                        result.addSelector(last_relation, new Selector(Selector.types.ID, 'id', found));
                                        st_pos = st_keywords; 
                                        break;
                                    }
                                    case KEYWORD_CLASS: {
                                        result.addSelector(last_relation, new Selector(Selector.types.CLASS, 'class', found));
                                        st_pos = st_keywords; 
                                        break;
                                    }
                                    case KEYWORD_TAG: {
                            ...
                            case MODE_GET_ELEMENT: {
                                if(last_keyword === KEYWORD_TAG) {
                                    result.addSelector(last_relation, new Selector(Selector.types.ELEMENT, 'tagName', found));
                                }
                            ...
                            case MODE_GET_ATTR: {
                                switch(last_keyword) {
                                    case KEYWORD_HAS: {
                                        result.addSelector(last_relation, new Selector(Selector.types.ATTR_HAS, 'attr', found));
                                        break;
                                    }
                                    case KEYWORD_EQUALS: {
                                        result.addSelector(last_relation, new Selector(Selector.types.ATTR_EQUALS, found));
                                        break;
                                    }
                                }
        ...
        result.each(function(selector_part) {
            var str_len,
                str;
