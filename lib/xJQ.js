    ...
    $.xJQ = function(selector, options) {
        ...
        if(selected.length > 1) {
            selected.each(function() {
                distance = content.distance(new ElementContent($(this)));
                
                if(typeof min_distance === 'undefined' || distance < min_distance.val)
                    min_distance = { val: distance, ref: $(this) };  
            });

            result = min_distance.ref;
            ...
            throw new Error('xJQ: element with selector `' + result_css_selector + '` cannot be found');
        }
